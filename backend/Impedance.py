...
from numpy import sqrt, real, imag, pi, tanh
...
def Z_film(f, film_args, d):
    ...
    res = sqrt(1j*2.0*pi*f*1e9*mu0/sigma(f) + 0*1j)/tanh(sqrt(1j*2.0*pi*f*1e9*mu0*sigma(f) + 0*1j)*d)
    return res
...
def Z_normal(f, sigma0, d, tau=0):
    ...
    res = sqrt(1j*2.0*pi*f*1e9*mu0/sigma + 0*1j)/tanh(sqrt(1j*2.0*pi*f*1e9*mu0*sigma + 0*1j)*d)
    return res
...
def Z_datasets(film_args, d, name):
    ...
    for freq in freq_arr:
        ...
        Z1 = sqrt(1j*2.0*pi*freq*1e9*mu0/sigma + 0*1j)/tanh(sqrt(1j*2.0*pi*freq*1e9*mu0*sigma + 0*1j)*d)
        res = Z1*(1.0 - 1j*pi*freq*1e9*mu0*d/Z1 + sqrt(1.0 + (1j*pi*freq*1e9*mu0*d/Z1)*(1j*pi*freq*1e9*mu0*d/Z1)))
