...
from numpy import abs, array, cos, cosh, exp, log, pi, sin, sinh, sqrt, tanh
...
def ZDAm(f, Wd, Ld, Ldo, Ed, D_base, PHASE=-1):
    ...
    XDA = (
        ...
        / 4.0
        * ((RD / rD) ** 2 - 1.0)
        * sin(2.0 * gamma_DCB * Ld)
        / (sin(gamma_DCB * Ld) ** 2 + (RD / rD * cos(gamma_DCB * Ld)) ** 2)
    )
    ZDA = RDA + 1j * XDA
    ...
    F3 = (
        lambda z: 2.0
        * cos(gamma_DCB * Ld)
        * exp(-1j * gamma_DCB * sqrt(D_base**2 + z * z))
        / sqrt(D_base**2 + z * z)
    )
