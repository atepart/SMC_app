...
from numpy import array, sqrt, real, imag, abs, exp, pi, arctanh, tanh, cosh, sinh, log
...
def chi_Kf1(W, H, d_top):
    ...
    eta = sqrt(p)*(pi*W/2.0/H + (p+1.0)/2.0/sqrt(p)*(1.0 + log(4.0/(p-1.0))) - 2.0*arctanh(1.0/sqrt(p)))
    
    rbo = eta + (p+1.0)/2.0*log(eta)
    rb1 = rbo - sqrt((rbo - 1.0)*(rbo - p)) + (p+1.0)*arctanh(sqrt((rbo-p)/(rbo-1.0))) - 2.0*sqrt(p)*arctanh(sqrt((rbo-p)/p/(rbo-1.0))) + pi*W*sqrt(p)/2.0/H
...
def Zo_s(f, W, E, H, top_s12_args, bot_s12_args, d_top, d_bot):
    ...
    res = 120.0*pi*g1/sqrt(E)*sqrt(1.0 - 1j*chi*(Z_top + Z_bot)/2.0/pi/f/1e9/120.0/pi/H/sqrt(eps0*mu0) + 0*1j)
...
def gamma(f, W, E, H, top_s12_args, bot_s12_args, d_top, d_bot):
    ...
    res = 1j*2.0*pi*f*1e9*sqrt(E*eps0*mu0)*sqrt(1.0-1j*chi*(Z_top + Z_bot)/2.0/pi/f/1e9/120.0/pi/H/sqrt(eps0*mu0))
