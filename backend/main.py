...
print(
    "Impedance of the top film at 650 GHz is equal to "
    + str(Imp.Z_film(650, (freq_arr, sigma1_top, sigma2_top), d_top))
)
...
W_ms = 9.5 * 1e-6
L_ms = 14.0 * 1e-6
M_DCB = lambda f: DCB.M_DCB(
    f, W_slot, L_slot, Lso, W_ms, L_ms, E_sub, H_sub, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot
)
...
Mex4 = lambda f: M_RAD(f).dot(
    MSL.M_MSL(f0, 3.0 * 1e-6, 6.0 * 1e-6 + DW, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot)
)


M_transf_1 = lambda f: ITR.Transf(
    f, 6.0 * 1e-6 + DW, 18.0 * 1e-6 + DW, 6.0, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot, dL=0.9e-6
)
M_MSL_4 = lambda f: MSL.M_MSL(f, 27.0e-6, 18.0e-6 + DW, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot)
M_transf_2 = lambda f: ITR.Transf(
    f, 18.0 * 1e-6 + DW, 4.0 * 1e-6 + DW, 2.0, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot, dL=0.9e-6
)
M_MSL_5 = lambda f: MSL.M_MSL(f, 10.0e-6, 4.0e-6 + DW, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot)
M_transf_3 = lambda f: ITR.Transf(
    f, 6.0 * 1e-6 + DW, 10.0 * 1e-6 + DW, 1.0, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot, dL=1.2e-6
)

Mex3 = lambda f: RES.group(f, (M_transf_1, M_MSL_4, M_transf_2, M_MSL_5, M_transf_3))


M_transf_4 = lambda f: ITR.Transf(
    f, 10.0 * 1e-6 + DW, 6.0 * 1e-6 + DW, 1.0, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot, dL=1.2e-6
)
M_MSL_6 = lambda f: MSL.M_MSL(f, 25.0e-6, 4.0e-6 + DW, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot)
M_transf_5 = lambda f: ITR.Transf(
    f, 6.0 * 1e-6 + DW, 18.0 * 1e-6 + DW, 3.0, E12, H12, top_s12_args, bot_s12_args, d_top, d_bot, dL=1.2e-6
)
...
M_transf_6 = lambda f: ITR.Transf(
    f, 18.0 * 1e-6 + DW, 48.0 * 1e-6 + DW, 3.0, E1, H1, top_s12_args, bot_s12_args, d_top, d_bot, dL=1.2e-6
)
M_MSL_8 = lambda f: MSL.M_MSL(f, 13.0e-6, 50.0e-6 + DW, E1, H1, top_s12_args, bot_s12_args, d_top, d_bot)
M_transf_7 = lambda f: ITR.Transf(
    f, 49.0 * 1e-6 + DW, 14.0 * 1e-6 + DW, 5.0, E1, H1, top_s12_args, bot_s12_args, d_top, d_bot, dL=1.2e-6
)
